...
public class MEPCrawler {
	...
	public static void main(String[] args) {
		String urlstr="http://datacenter.mep.gov.cn/report/air_daily/air_dairy_aqi.jsp?city=&startdate=&enddate=&page=7187";
		...
		if(args.length==4){
			...
			urlstr="http://datacenter.mep.gov.cn/report/air_daily/air_dairy_aqi.jsp?city=&startdate=&enddate=&page="+args[0];
			outputlocation=args[1];
			System.out.println("outputlocation : "+outputlocation);
		}else if(args.length==1){
			urlstr="http://datacenter.mep.gov.cn/report/air_daily/air_dairy_aqi.jsp?city=&startdate=&enddate=&page=1";
		...
		for(int i=0;i<pageResultCollector.getCollected().size();i++){
			...
			while (iter.hasNext()) {
				...
				writer.print(item.getPI()+":");
				writer.print(item.getPollution()+":");
				writer.print(item.getAIRQL()+":");
				writer.println(item.getAIRQS());
			}
		}
		writer.close();
